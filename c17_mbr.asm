		 ...
		 core_base_address equ 0x00040000   ;常数，内核加载的起始内存地址 
		 core_start_sector equ 0x0000000F   ;常数，内核的起始逻辑扇区号		 
...
SECTION  mbr  vstart=0x00007c00         
         ...
         ;以下进入保护模式... ...
         jmp dword 0x0008:flush             ;16位的描述符选择子：32位偏移       汇编地址：0x7c5f
                                            ;清流水线并串行化处理器
         [bits 32]               
  flush:                                  
         ...
         mov ebx,edi                        ;起始地址
         call read_hard_disk_0              ;以下读取程序的起始部分（一个扇区）,将程序加载到DS:EBX处（内存0x00040000处）
   ...
   pge:
         ...
         ;创建系统内核的页目录表PDT
         mov ebx,0x00020000                 ;页目录表PDT的物理地址    汇编地址：0x7cb0
  ...
  .b1:       
         ...
         mov cr3,eax                        ;将页目录表基地址赋值给cr3寄存器

         ;将GDT的线性地址映射到从0x80000000开始的相同位置 
		 ;sgdt：将GDTR中的基地址和边界值保存到内存中，lgdt：从内存中取6个字节（前两个字节边界值，后4个字节基地址）加载到GDTR中
         sgdt [pgdt]                        ; 汇编地址：0x7cf3
         mov ebx,[pgdt+2]                   ; ebx=0x00008000
         add dword [pgdt+2],0x80000000      ;GDTR也用的是线性地址  0x80008000
         lgdt [pgdt]

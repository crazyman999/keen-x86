...
;-------------------------------------------------------------------------------
		 ...
		 user_program_01        equ  0x32        ;c17-1程序所在扇区(没有使用)
		 user_program_02        equ  0x64        ;c17-2程序所在扇区(没有使用)
;-------------------------------------------------------------------------------          
         ;以下定义宏
...
;------------------------------------------------------------------------------- 
         ...
         ;任务控制块链
         tcb_chain        dd  0             ;四个字节保存的是core_tcb的标号，也就是下面32个字节的地址
...
load_relocate_program:                      ;加载并重定位用户程序
         ...
         mov ebx,0xfffff000
         xor esi,esi
  .b1:                                         ; ds=0x10  ss=0x10    [基地址为0，界限0xFFFFF，DPL=00,0-4G段]
         ...
         mov ebx,core_buf                   ;读取程序头部数据，内存缓冲区512字节，读取用户程序的第一个扇区数据，保存在这里
         ...
         mov esi,[ebp+36]                   ;从堆栈中取得TCB的基地址  【第一个用户程序执行时 esi=0x80101000】
  .b2:
...
start:
         ...
         ;此中断门处理前20个向量，处理器异常使用的 【P342】
         mov eax,general_exception_handler  ;门代码在段内偏移地址
         mov bx,flat_4gb_code_seg_sel       ;门代码所在段的选择子  【0x0008】
         mov cx,0x8e00                      ;32位中断门，0特权级,  【10001110】   P|DPL|0D110
         call flat_4gb_code_seg_sel:make_gate_descriptor     ; 0x0008:xxx   创建中断门  ; 返回：EDX:EAX=完整的描述符

         mov ebx,idt_linear_address         ;中断描述符表的线性地址    【0x8001f000】  1000000000|0000011111|000000000000
         xor esi,esi                        ;汇编地址 0x40db8
  .idt0:
         mov [ebx+esi*8],eax            ; make_gate_descriptor  返回：EDX:EAX=完整的描述符
         mov [ebx+esi*8+4],edx          ; make_gate_descriptor  返回：EDX:EAX=完整的描述符
         inc esi
         cmp esi,19                         ;安装前20个异常中断处理过程
         jle .idt0

         ;其余为保留或除CPU之外的硬件使用的中断向量
  ...
  .idt1:
         ...
         mov bx,flat_4gb_code_seg_sel       ;门代码所在段的选择子
         mov cx,0x8e00                      ;32位中断门，0特权级    10001110
         call flat_4gb_code_seg_sel:make_gate_descriptor

         mov ebx,idt_linear_address         ;中断描述符表的线性地址
         mov [ebx+0x70*8],eax               ;0x70号向量处理时钟中断
         mov [ebx+0x70*8+4],edx

         ;准备开放中断
         mov word [pidt],256*8-1            ;IDT的界限
         mov dword [pidt+2],idt_linear_address
         lidt [pidt]                        ;加载中断描述符表寄存器IDTR    汇编地址：0x40e28
  ...
  .b4:
         ...
         mov cx,1_11_0_1100_000_00000B      ;特权级3的调用门(3以上的特权级才
                                            ;允许访问)，0个参数(因为用寄存器
                                            ;传递参数，而没有用栈) 
         call flat_4gb_code_seg_sel:make_gate_descriptor   ; EDX:EAX   8004ec00:00080008
         call flat_4gb_code_seg_sel:set_up_gdt_descriptor  ; set_up_gdt_descriptor 输出：CX=描述符的选择子, 为每个函数各自建一个调用门
         ...
         ;对门进行测试 
         mov ebx,message_1                  ; 汇编地址：0x40f2a
         call far [salt_1+256]              ; 通过门显示信息(偏移量将被忽略) 

         ;初始化创建程序管理器任务的任务控制块TCB, core_tcb：内核（程序管理器）的TCB  32字节(因为是内核，特权级是0，因此不需要建3、2、1级栈的基地址、选择子、TSS等信息，32字节足够)
         mov word [core_tcb+0x04],0xffff    ;任务状态：忙碌 （0xffff为忙碌是我们自己程序定义的，跟硬件无关）  ds=0x10
         mov dword [core_tcb+0x06],0x80100000    ; 程序加载线性基地址
                                            ;内核虚拟空间的分配从这里开始。
         mov word [core_tcb+0x0a],0xffff    ;登记LDT初始的界限到TCB中（未使用）
         mov ecx,core_tcb                   ; ecx = 0x800402de     core_tcb、tcb_chain 的值参见 core_tcb_1.jpg 
         call append_to_tcb_link            ;将此TCB添加到TCB链中。core_tcb、tcb_chain 的值参见 core_tcb_2.jpg      
                                            ;汇编地址：0x40f56
         ;为程序管理器的TSS分配内存空间
         alloc_core_linear                  ;宏：在内核的虚拟地址空间分配内存。core_tcb、tcb_chain 的值参见 core_tcb_3.jpg  
                                            ;[core_tcb+0x06]的值从0x80100000 变成了 0x80101000 ，分配了一个4k页，程序加载线性基地址加了0x1000
         ;在程序管理器的TSS中设置必要的项目 
         mov word [ebx+0],0                 ;反向链=0   ebx=0x80100000【程序加载线性基地址】
         mov eax,cr3
         ...
         mov ecx,0x00408900                 ;TSS描述符，特权级0
         call flat_4gb_code_seg_sel:make_seg_descriptor
         call flat_4gb_code_seg_sel:set_up_gdt_descriptor  ;set_up_gdt_descriptor 输出：CX=描述符的选择子
         mov [core_tcb+0x18],cx             ;登记内核任务的TSS选择子到其TCB

         ;任务寄存器TR中的内容是任务存在的标志，该内容也决定了当前任务是谁。
         ;下面的指令为当前正在执行的0特权级任务“程序管理器”后补手续（TSS）。
         ltr cx                ; 汇编地址：0x40fb0。;LTR指令从GDT中取出相应的TSS段描述符，把TSS段描述符的基地址和界限等信息装入TR的高速缓冲寄存器中。注意它并不会执行任务切换。
                               ;这个地方实际上是在模拟从另一个任务切换到当前任务（程序管理器任务）的情况。TR寄存器中就应该保存“程序管理器”这个任务的TSS
         ;现在可认为“程序管理器”任务正执行中
         ...
         push ebx                           ;压入任务控制块起始线性地址[0x80101000]   汇编地址：0x40fdf
         call load_relocate_program         ; esi=0x100  sp=0x80006ffc   ebx=0x80101000
         mov ecx,ebx         
